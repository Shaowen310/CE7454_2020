...
def run_epoch(data_iter, model, loss_compute):
    ...
    for i, batch in enumerate(data_iter):
        ...
        if i % 50 == 1:
            ...
            print('Epoch Step: {} Loss: {:.4f} Tokens per Sec: {:.4f}'.format(
                i,
                loss.detach().float().numpy() / batch.ntokens.float().numpy(),
                float(tokens) / float(elapsed)))
            print('               t1: {:.4f} t2: {:.4f}'.format(loss.detach().float().numpy(),
                                                                batch.ntokens.float().numpy()))
            start = time.time()
            tokens = 0
